function [Xs,out] = WNewton_aff_mod(X_init, d_hess_log_p, opts)
%%%---------------------------------------------%%% 
% This implements the modified affine Wasserstein Newton's method
% 
% Input:
...
%%%---------------------------------------------%%%
	...
	for iter = 1:iter_num
		...
		F = -mean(Hess,3)+epsl*I;

		X_nor = X-X_mean;
		v_nor = v-v_mean;
		Sigma_ = (X_nor'*X_nor)/N;
		Xv =  (X_nor'*v_nor)/(2*N);
		T = -Xv-Xv';

		CGopts = struct('max_iter',1e3,'crit',1,'tol',1e-6,'S',S);
		S = Gaussian_CG(Sigma_,F,T,CGopts);
		if opts.bm
			X = X+tau*(X_nor*S-v_mean*inv(F)+lbd*grad)+sqrt(tau*lbd)*randn(size(X));
		else
			X = X+tau*(X_nor*S-v_mean*inv(F)-lbd*v);
		end
