function [Xs,out] = HALLD_m(X_init, dhess_log_p, opts)
%%%---------------------------------------------%%%
% This implements the time discretization of HALLD.
% 
% Input:
% 		X_init --- initial particle positions, N*d matrix
%		dhess_log_p --- function handle to compute the derivative and Hessian of log p(x)
...
%
%				   avg --- whether to average the Hessian
%
%
...
%%%---------------------------------------------%%%
	...
	if ~isfield(opts, 'avg'); opts.avg = 1; end 
	...
	for iter = 1:iter_num
		...
		if ktype == 0
			if opts.avg
				Fisqrt = sqrt(Finv);
				X = X+tau*grad*Finv-sqrt(2*tau)*randn([N,d])*Fisqrt;
			end
		else
			[xi, ibw_out] = dlog_p_prac(X,ktype,kopts);
			ibw = ibw_out;
			if opts.avg
				X = X+tau*(grad-xi)*Finv;
			else
				for i = 1:N
					X(i,:) = X(i,:)+tau*(grad(i,:)-xi(i,:))*inv(Hess(:,:,i));
				end
			end
		end
