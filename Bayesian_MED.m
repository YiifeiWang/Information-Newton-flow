...
train_ratio = 0.8;
max_iter = 4000;  % maximum iteration times
...
dhess_log_p  = @(theta)dhess_log_p_Bayesian(theta, X_train, y_train,batchsize,a0,b0); % returns the first order derivative of the posterior distribution 

SVGD.name = 'SVGD';
SVGD.Pname = 'SVGD';
SVGD.opts = struct('tau',0.05,'iter_num',max_iter,'record',1,'trace',1,'ibw',-1,'ktype',1,...
...
Wnes.name = 'WNAG';
Wnes.Pname = 'WNes';
Wnes.opts = struct('tau',1e-5,'iter_num',max_iter,'record',1,'trace',1,'ibw',-1,'ktype',1,...
		'itPrint',1e2,'X_test',X_test,'y_test',y_test,'tau_itv', 1e2, 'tau_dec',0.9,'wtype',2,'ttype',2);

MCMC.name = 'MCMC';
MCMC.Pname = 'MCMC';
MCMC.opts = struct('tau',1e-5,'iter_num',max_iter,'record',1,'trace',1,'ibw',-1,'ktype',0,...
	'itPrint',1e2,'X_test',X_test,'y_test',y_test,'tau_itv', 1e2, 'tau_dec',0.9);

WGF.name = 'WGF';
WGF.Pname = 'WGF';
WGF.opts = struct('tau',1e-5,'iter_num',max_iter,'record',1,'trace',1,'ibw',-1,'ktype',1,...
	'itPrint',1e2,'X_test',X_test,'y_test',y_test,'tau_itv', 1e2, 'tau_dec',0.9,'ttype',1);

AIG.name = 'AIG-SE';
AIG.Pname = 'AIG';
AIG.opts = struct('tau',1e-6,'iter_num',max_iter,'record',1,'trace',1,'ibw',-1,'ktype',1,...
	'itPrint',1e2,'X_test',X_test,'y_test',y_test,'tau_itv', 1e2, 'tau_dec',0.9, 'restart',1,'strong',0,'ttype',1);

WNewton.name = 'WNewton';
WNewton.Pname = 'WNewton';
WNewton.opts = struct('tau',2e-3,'iter_num',max_iter,'record',1,'trace',1,'ibw',-1,'ktype',1,...
			'itPrint',1e2,'X_test',X_test,'y_test',y_test,'tau_itv', 1e2, 'tau_dec',0.82,'ttype',1,'epsl',0 ...
			,'lbd',5e-3,'ptype',1);

mWNewton.name = 'mWNewton';
mWNewton.Pname = 'mWNewton';
mWNewton.opts = struct('tau',2e-3,'iter_num',max_iter,'record',1,'trace',1,'ibw',-1,'ktype',1,...
			'itPrint',1e2,'X_test',X_test,'y_test',y_test,'tau_itv', 1e2, 'tau_dec',0.82,'ttype',1,'epsl',0 ...
			,'lbd',5e-3,'ptype',1);

algorithm = {MCMC,SVGD,Wnes,WGF,AIG,WNewton,mWNewton};
results = {};
	...
	for i = 1:length(algorithm)
		...
		switch algorithm{i}.name
			...
			case 'MCMC'
				[~,out] = WGF_m(theta0,dlog_p,algorithm{i}.opts);
			case 'WNewton'
				[~,out] = WNewton_aff_diag(theta0,dhess_log_p,algorithm{i}.opts);
			case 'mWNewton'
				[~,out] = WNewton_aff_mod(theta0,dhess_log_p,algorithm{i}.opts);
		end
